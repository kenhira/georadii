"""
Code to generate archive files - separate files for png and nc
"""
...
for subdir in sorted(all_subdirectories):
    ...
    for png_file in png_file_all:
        ...
        png_file_newbase = 'ARCSIX-Imagery-Nadir-Quicklook_P3B_%s%s_R0.png' % (date, st_time)
    ...
    for ig in range(n_groups):
        ...
        new_zip_path_png = 'ARCSIX-Imagery-Nadir-Quicklook_P3B_%s%s_R0.zip' % (date, st_time_png)
        print('Writing to a zip file:', new_zip_path_png)
        with zipfile.ZipFile(new_zip_path_png, "w", zipfile.ZIP_DEFLATED) as zp:
            print('png...')
            for png_file in png_file_in_group:
                zp.write(png_file, arcname=os.path.basename(png_file))
        
        new_zip_path_nc = 'ARCSIX-Imagery-Nadir_P3B_%s%s_R0.zip' % (date, st_time_png)
        print('Writing to a zip file:', new_zip_path_nc)
        with zipfile.ZipFile(new_zip_path_nc, "w", zipfile.ZIP_DEFLATED) as zp:
            print('nc...')
